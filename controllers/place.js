...
exports.visits = async (req, res) => {
	...
	res.render("place/visits", {
		...
		pages,
		count,
		paginationLinkUrl: `/place/${req.params.id}/visits/page/`,
		queryString,
	});
};
...
exports.visit = async (req, res) => {
	...
	res.render("place/visit", {
		pageTitle: singlePlace.name,
		place: singlePlace,
		visit: singlePlace.visits,
	});
};
...
exports.updateVisit = async (req, res) => {
	...
	await Place.updateOne(
		...
		{
			$set: {
				"visits.$.comment": req.body.visit.comment,
				"visits.$.cost": req.body.visit.cost,
				"visits.$.rating": req.body.visit.rating,
			},
		}
	);

	await Place.findById(placeId, async (err, place) => {
		const ratingTotal = place.visits.reduce((sV, cV) => ({
			rating: sV.rating + cV.rating,
		}));
		const costTotal = place.visits.reduce((sV, cV) => ({
