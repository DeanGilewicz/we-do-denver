class CardActions {
	constructor({
		...
		if (
			...
			typeof multiInputContainer !== "undefined"
		) {
			this.multiInputContainer.addEventListener(
				"focus",
				(e) => {
	...
	_deleteInput() {
		...
		const inputsEnd = inputsArray.slice(
			this.inputIndex + 1,
	...
	_addInput() {
		...
		additionalInput.classList.add(
			"form__input",
			`form__input--${this.attrValue}`
		);
		additionalInput.setAttribute(
			"name",
			`${this.attrValue}-${this.formInputs.length}`
		);
		...
		this.formInputs = this.multiInputContainer.querySelectorAll(
			`[data-input="${this.attrValue}"]`
		);
		// focus in last input
	...
	_enableInput() {
		if (
			this.formInputs.length > 0 &&
			typeof this.multiInputContainer !== "undefined"
		) {
	...
	_resetInputValue() {
		if (
			...
			// update DOM
			this.multiInputContainer.innerHTML = this.formInputs
				.map((el, i) => {
